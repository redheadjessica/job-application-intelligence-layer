...
- ⭐ **A LIVING number goes stale by getting bigger — never copy one forward from an older resume.** Most metrics are frozen the day the work ended (a past role's revenue figure is what it was). But some describe something the candidate is *still building* — codebase size, users, shipped features, months of operation — and those only grow. Copying one from a prior resume silently understates the candidate, and understatement is invisible: nobody reviewing the draft sees the number that *should* have been there. Treat a living number as a lookup, not a constant: re-read it from the canonical source (the project's `03*-canonical.md`), check the measurement's date, and if it is stale and load-bearing for this role, say so in the review packet rather than shipping the old figure. When there is no fresh measurement, prefer a scale-free phrasing over a number you cannot stand behind — it never goes stale and never undersells.
