...
**⭐ FILL THE LINE — aim for the mid-to-high teens.** This is a paste-whole deliverable on a space-constrained page, and an under-filled line wastes the single densest keyword surface on the résumé. Do not stop at "enough to cover the JD" — cover the JD's named requirements in its own vocabulary FIRST, then keep going with the candidate's genuine breadth terms until the line is full. A dozen or so items reads thin — one candidate reviewing a regenerated batch called a 13-item line "too short by 1" and another "too short by 3 to 5." The output-contract check enforces a hard floor, configurable per candidate via `resume.min_skills_items` in `jail.config.json` (default 12), because how full "full" is depends on résumé layout and term length. Treat the floor as the failure line, never the target.
...
- **⭐ Resolve against the BASE's own folder, never the job folder.** Pass the base path from `02-resume-index.md` — the file in the candidate's résumé library — and the resolver finds its sibling PDF there. The job folder holds only your fresh copy of the base and never a PDF (see the one-artifact rule in Step 5), so looking there will always come up empty. Real failure this prevents (08-08-26): a run reported `no-readable-pdf` and dropped all four comparison cells for a base whose exact-name PDF was sitting in its own folder the whole time; running the resolver against the index path returned `status: ok` immediately. Before reporting `no-readable-pdf`, you must have actually run the command above against the index path and seen that status come back.
- **`no-readable-pdf` / `not-found` → do not score, and do not estimate.** Omit `base_resume_score`, `improved_resume_score` and `why_it_improves` from your return entirely, leaving all four tracker cells blank, and state plainly in **Final Risks / Notes** which file was missing and what to export — quoting the resolver's own output, so a wrong "missing" claim is visible. A number guessed from the index is indistinguishable in the spreadsheet from one measured off the document.
...
- **⭐ Deep canonical reference files — `03*-canonical.md` (e.g. `03-ascend-canonical.md`, `03c-ridg-canonical.md`).** These are **cold files**: rich, long, and NOT needed on a normal run.
  - **You do not need them to write approved bullets.** `04-experience-bank.md` — already in the core batch — is the single authoritative home for every approved bullet's TEXT, plus its variants and the bullet-count routing by company type. Canonical files contain **no bullet text at all**. On a routine run where approved bullets cover the JD, reading them adds nothing and costs a great deal (they run 30KB+ each, against a ~315KB core batch, and the biggest one applies to a role that appears on nearly every resume — a "conditional" read that fires every time is just an expensive core file in disguise).
  - **Read one ONLY when a concrete trigger fires** — not on a judgment call about whether the experience "features prominently," which is vague enough to erode into always or never:
    1. you are about to write a **Suggested New** bullet for that experience;
    2. **no approved bullet covers what the JD is asking for**, and you need the wider evidence base to compose one;
    3. you need a specific fact the bank does not carry — a boundary, an attribution split, a confidentiality rule, or a maintained figure (e.g. Ascend's codebase-size table, which must be read fresh and never carried over from an older resume);
    4. maintenance, audit, or reconcile work.
  - **When one does fire, honor the file's evidence tags.** These files mark claims by strength (proven / co-attributed / testimony / not supported) and carry explicit do-not-claim lists and confidentiality rules. Those bind: a "not supported" item never becomes a bullet, however well it fits the JD. Composition still follows the normal rules — recompose approved descriptors, no new factual claims, and flag any net-new bullet **Suggested New**.
  - **One home per bullet, enforced.** Bullet text lives only in the experience bank; canonical files reference it and never restate it. This is not a style preference — the two copies drifted once already, canon corrected the Ascend opening bullet on 7/20/26, the bank's copy was missed, and the always-read file shipped retired wording for eleven days. `ENGINE__PUBLIC_GIT_TRACKED/04-TAILOR/check_canon_drift.py` detects a recurrence.
  - **Deep-context companions (e.g. `03b-ascend-north-star.md`) are NOT part of generation.** They exist to seed cover-letter narrative and interview prep. Do not open them during a tailoring run.
