...
- The candidate **never edits the generated `.docx`** — it is always, verbatim, the agent's final output. The equivalent markdown baseline is `_JAIL Agent Work/final.md` in the job folder — with a fallback to the legacy `_cl_work/final.md` for folders created before the 2026-08-04 rename. Readers prefer the new path and fall back; writers only ever emit the new one.
