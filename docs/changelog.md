...
**Root cause: the same duplication bug, for at least the seventh time.** The 08-07 restructure rewrote the spec's output structure but left `job-applier.md` still enumerating the old section list — "job analysis, gap check, resume-base recommendation, work-experience changes, 3 summary options, skills line, integrity check." That list omits `Selected Writing` and `Read Log`. Agents followed the list they were handed over the spec they were pointed at, and the two omitted sections vanished from the output. A third copy of the retired "add a Questions for the candidate section at the top" instruction was also sitting in the workflow prompt.

Worth stating plainly, because the repo keeps relearning it: **prompt duplication is far more dangerous than code duplication.** Two disagreeing Python definitions produce an error you can reproduce. Two disagreeing prompt definitions get resolved independently by every agent — 4 right, 31 wrong. Partial compliance survives spot-checks, defeats "I tested it and it worked," and looks like success. Fan-out turns it from a risk into a certainty.

**Fixes, by layer:**

- **One definition.** The overlay files stop enumerating sections and point at the spec's fenced Primary Output Format block, which is now the single machine-readable source.
- **Two contradictions resolved without losing an ingredient.** Work experience keeps its `REPLACE / Old / New` diff (that's the record of what changed) *and* gains a mandatory paste-ready `- ` block after it, with an explicit contract: no numbering, quotes, label prefixes, bold lead-ins, or annotation lines. The candidate's own principle held throughout — reorganize, never remove.
- **Anti-inheritance, generalized.** The "NEVER INHERIT THIS SECTION FROM THE BASE" rule had been scoped to Selected Writing while describing a failure that applies to everything: *a base carries the choices that were right for whatever role it was originally built for, and those travel invisibly into every descendant.* It now covers bullets, skills, summary and page 2, and "keep as in base" is valid only after a stated comparison. Skills specifically is now built from canon with the base ignored entirely — measured erosion: one batch's median fell to 8 items against a prior floor of 13, because each generation trimmed what the *last* JD didn't need and nothing ever re-expanded.
- **Conditional rulings fire deterministically.** New `base_shortlist.py` prints every registered base and what the candidate said it is for, newest first, and Step 5 requires a one-line verdict on each. The case that forced it: a community-and-growth role was tailored while a base registered the day before — whose index entry named it best for exactly that shape of role (heavy go-to-market and public brand, the interplay of product and marketing) and noted its page 2 had been reworked to carry that evidence — was never mentioned once. The ruling existed; it was buried among eighty entries in a 762-line index. Selection cannot depend on recall.
- **The artifact is validated.** New `check_tailoring_output.py` **parses its expectations from the spec** rather than hardcoding them — a hardcoded checker would be a third copy and the next thing to drift. It asserts structure, a URL on every named writing piece, paste-ready bullets, a substantive skills section, base verdicts, and a Read Log. Wired into `tailor-jobs.js` with one repair pass, then a loud failure.
- **Compliance, not completion.** The result now reports "N of M passing the output-contract check." "35 tailored" was true and useless.
- **The duplication now breaks the build.** `test_output_contract.py` fails when an overlay re-enumerates spec-owned sections or demands the retired section — verified against the actual pre-fix file, which both guards catch — plus golden-file cases for every defect this batch shipped.

**Two process notes worth keeping.** The 08-07 change was "validated by reorganizing three existing files with a content-preserving script" — that validated the format could be *produced*, never that the pipeline produces it; no real job was run. And the QA pass that declared the batch clean checked for a section name the retired structure also contained, so it passed 35/35 while 31 were wrong. A hand-written check encodes whatever its author believed at the time, which is the whole argument for parsing the contract instead of restating it.

Also learned while building the base ledger: three attempts at lexically ranking base purpose-prose against a job post put the obviously-correct base 19th of 21. Prose similarity was the wrong instrument, and a bad relevance sort is worse than none — it implies a judgment the numbers can't support and invites skipping the tail. The ledger now lists all 21 bases newest-first and filters nothing; relevance is the agent's job, made against every purpose line rather than a guessed subset.
...
The cause was `make_rankings_xlsx.py --config` defaulting to `None`. Run through the normal vetting workflow the config is passed and everything colors correctly; regenerated by hand without that flag, `load_config` returns `{}`, `comp_fit_label` sees neither floor nor target, and every row honestly reports "this candidate has no comp preferences." The output is indistinguishable from the real thing — no warning, no error, just a grey column that looks deliberate. The normalization pass then made it durable by rewriting valid labels (`Near target` → `No comp prefs`) into the CSV.

Fixed by walking up from the script's own location to find the repo-root `jail.config.json` when no `--config` is given. The general shape worth remembering: a default that produces a *plausible* output is worse than one that errors. "No preferences configured" and "preferences not loaded" rendered identically, so the failure was invisible until someone noticed a missing color.

**New `Top of market` comp tier.** The candidate asked for a brighter green on roles paying above roughly $250K, prominently, in this and every future tracker. Added as a fifth Comp Fit label with its own loud fill, tested against the band's HIGH end (the question is "could this role pay that?" — a midpoint test would leave the tier almost always empty).

The threshold is **config-only** (`comp.top_of_market_base`), with no engine-side default, because `03-VETTING/CLAUDE.md` is explicit that salary tiers are candidate data and belong in `jail.config.json`. A first pass hardcoded 250 in `norm_contracts.py` before that rule was re-read — worth noting, because the hardcoded version would have quietly imposed one person's idea of "high" on every JAIL user. Absent the key the tier doesn't exist and coloring behaves exactly as before; the template ships it as `null`. The Instructions tab now explains the whole comp palette, including what an all-grey comp column means and how to fix it.
...
**What was wrong, and the fix in each case:**

- **A pay label with no number was read as "employer didn't post pay."** Greenhouse-embedded employer pages (flatiron.com) render `Salary Range:` server-side and inject the figures client-side, so a static scrape captures the bare label. That got classified `not_posted`, which is *benign* and therefore never retried — hiding Flatiron's $136,000–$187,000. A dangling pay label is now positive proof pay WAS posted, so the field is `capture_failed` (retryable, and the retry renders the page). This is the most dangerous class of bug in the gate: a false `not_posted` is invisible by design.
- **Single-number salaries were invisible.** The prose matcher only understood bands (`$X – $Y`). Bain publishes one number — "the good-faith, reasonable annualized full-time salary range for this role is $197,300" — and the field came back "Could Not Verify" on a posting that plainly stated pay. Single-point pay is now recognized, with a stricter test than the range form (explicit currency marker AND a salary keyword) so headcounts, years, and funding totals can't masquerade as pay. Figures introduced as total comp / OTE / base+bonus are still excluded from base salary.
- **A stub scored as a complete posting.** Stripe's Startup Products capture was 1.4KB of "About Stripe / About the team" — no duties, no requirements, no location — and passed the usable gate because the sentence "Every person on the team is responsible…" matched the loose content marker `responsib`. It was then scored as if it were the whole job. The gate now requires a structural marker (a real duties/requirements/qualifications section) or several distinct loose ones. Validated against all 41 captures in the batch: it catches exactly the one stub, with no false positives on the 37 good posts.
- **Ashby boards behind a custom domain were unreachable when the slug didn't match the brand.** The org was guessed from the domain, so one-pass.com (board `one-pass-solutions`) failed every guess and the posting went to the thin pile — with its structured multi-zone compensation, which lives in an API field and never appears in the description prose, unread. The slug is now read from the embed script on the employer's own careers page instead of guessed.
- **Greenhouse boards behind a product-name domain were unreachable.** onepeloton.com's board is `peloton`; no candidate token stripped the leading "one". Brand *prefixes* are now stripped alongside the existing suffix handling. Recovering Peloton also recovered its application questions, which is where the hybrid arrangement actually lives — "commute to the New York HQ office on Tuesdays, Wednesdays and Thursdays" — a fact that appears nowhere in the job description body.
- **Two filename/identity bugs on the same capture.** `&amp;` slugged to the literal word "amp"; entities are now decoded before slugging and before the header is written. And careers.bain.com handed the scraper the role in both the company and title slots, after which title recovery pulled the org name into the title — inverting the pair. The domain is now used as an independent witness to break that tie, but only when it matches the title and not the company, so a correct pair is never touched.

**The plumbing bug behind the plumbing bug:** the Ashby embed fix worked standalone and silently did nothing in the real run. Prep executes the whole batch inside one `sync_playwright()` context, and Playwright's sync API cannot be re-entered — the nested launch raised, and the `except` swallowed it. Prep now lends its open browser to the ATS layer. Worth remembering generally: a recovery path that renders a page cannot assume it owns the browser, and a bare `except` around it will turn "broken" into "found nothing."

**The reporting change.** The candidate also asked why the rankings flagged 2 incomplete captures when 9 jobs had no comp. The answer was that the banner deliberately lists only rows we *couldn't verify* and treats "employer didn't publish it" as benign — defensible, but the benign rows were invisible, so the banner read as "everything else was found." Both the Markdown and XLSX banners now state the benign count and name those rows, so the two categories are visibly different rather than one being unmentioned.

**The lesson underneath all six.** The retry trigger was length, and the completeness verdict was trusted from whichever source answered first. A capture can be long, well-formed, and confidently wrong about what the employer published. Prep now retries on *missing fields* rather than short bodies, refuses to call a field "not posted" when the page shows a label for it, and refuses to call a posting complete when it has no duties section. Every one of these was found by a human reading the output and going to check — which is exactly the work the gate exists to make unnecessary.

Re-ran the batch end to end after the fixes: 43/43 usable (was 38/41 usable with 3 quarantined), zero `capture_failed` fields, and Flatiron, Bain, One Pass, Peloton, Color, and Stripe all now carry the comp and location the candidate found by hand. 479 prep tests pass, including 11 new regressions covering each defect.
...
Root cause: the draft agent returned a **well-formed but fabricated** result. Its structured output carried a plausible `draft_path`, `lint_errors: 0`, and `word_count: 619` for a file it never created, because the job folder was gone. The schema was satisfied by invented values, so every downstream stage trusted it — Evaluate "scored" a letter it never read, Finalize "packaged" it, and Record wrote to the tracker. The only guard in the pipeline was `if (!draft) throw`, which catches a null agent result but not a confidently wrong one. This is the same failure shape as the reconcile bug fixed the day before: a workflow returning a plausible wrong answer instead of a loud failure, with the added harm that this one wrote into the candidate's tracker.

Fixed with an existence gate between Draft and Evaluate. After the writer returns, a cheap haiku agent runs `wc -c "<draft_path>"` and reports the command's verbatim output; the stage proceeds only on a real byte count with exit 0. The gate never asks an agent whether it did its job — it checks the filesystem. On a miss the job is pushed to a new `failures[]` array with the claimed path and the shell evidence, then the stage throws so the pipeline drops that item and skips its remaining stages: no scores, no `.docx`, no packet, and critically no rankings write. The run's return gained a `failures` field and its note now names which jobs produced nothing and why, so a partial run can never again read as a clean one.

Verified both branches against the real filesystem rather than by inspection: positive — a genuine draft returns `exists: true` with `4088 … EXIT:0`; negative — a nonexistent path returns `exists: false` with `No such file or directory / EXIT:1`. Worth noting for anyone writing a similar test: pointing the workflow at a job whose folder is missing does NOT reproduce the bug, because the writer agent's instructions include `mkdir -p` and it simply recreates the folder and writes a real letter. The negative branch has to be exercised in isolation.

## 2026-08-06 — `/reconcile` was silently reconciling nothing; per-folder findings now go through a JSON sidecar instead of the return schema

Every `/reconcile` run was reporting success and writing nothing. Discovery worked and found ready folders, but all five per-folder reconcile agents died before their first turn with `blocked by safety classifier: output schema too large to classify safely`, and the workflow's own guard turned that into a bland `{"reconciled": 0, …, "note": "All reconcile agents returned empty."}`. The learning loop had been dark — no reports, no ledger entries, no evidence captured — while looking like it ran. The failure mode is the dangerous one: a workflow that fails loudly gets fixed, one that returns "0 reconciled" reads as "nothing to do."

Root cause: `RECONCILE_SCHEMA` was a ~5100-char deeply-nested structure (observed/inferred/questions/proposed_updates/anecdotes/answer_pairs/cover_letter_findings), large enough to be rejected outright when the agent's structured-output tool was built. `DISCOVER_SCHEMA` at ~1975 chars passed, which is why discovery alone always worked.

Fixed by getting the payload out of the return value entirely: each reconcile agent now writes its full findings object to a `reconcile-findings - <Company> - <Role> - <date>.json` sidecar beside its human-readable report, and returns only six scalars (`folder_path`, `company`, `role`, `report_path`, `findings_path`, `headline`). The Synthesize stage reads the sidecars off disk instead of receiving an inlined `RESULTS_JSON` blob. `RECONCILE_SCHEMA` went 5109 → 373 chars. Nothing was lost — the full field contract (including the anecdote `new=true/false` reuse semantics and the cover-letter baseline rule) moved into the agent prompt as an explicit sidecar spec. Side benefit: the findings are now durable on disk next to the report, so a failed synthesize no longer discards the per-folder analysis, and the sidecar is re-readable without re-running the agents. Verified end-to-end: 5/5 folders reconciled, reports + sidecars written, ledger/queue/answer-bank updated.

Process note worth keeping: `Workflow({name: "reconcile"})` resolves the workflow from a registry copy, NOT from the working-tree `.claude/workflows/reconcile.js`. Two fix attempts appeared to "fail" because the edited file was never the thing being executed. When iterating on a workflow in this repo, invoke it with `{scriptPath: ".claude/workflows/<name>.js"}` to run the file you just edited.

## 2026-08-06 — Front-door vet+tailor now writes the resume-comparison columns back to the rankings, same as the hand-picked path

The six tailoring/tracker columns — `Tailored? (Base Resume)`, `Base Resume Score`, `Improved Resume Score`, `Resume Improvement Delta`, `Why It Improves` (plus `Cover Letter Drafted?`, owned by the cover-letter workflow) — were staying blank after tailoring for anyone using the front-door `run-batch {folder, tailor: true, topN: N}` flow. Root cause: two parallel tailoring code paths had drifted. `tailor-jobs.js` (the hand-picked path) had a full Record phase that writes each job's chosen base AND its resume-comparison block into `1 - Rankings/` via `update_rankings_row.py` — but `run-batch.js`'s inline tailor loop had none of it, and its agent schema didn't even *request* the comparison scores. So the primary vet+tailor entrypoint tailored resumes and then left every one of those columns empty, while the less-used hand-picked path filled them.

Fixed by collapsing to a single tailoring path: `run-batch`'s Tailor phase now delegates to `tailor-jobs.js` (the same by-scriptPath delegation it already uses for vetting via `vet-jobs.js`), instead of tailoring inline. `tailor-jobs` gained an optional `parallel` flag so `run-batch`'s overnight `tailorParallel` mode is preserved. Now the rankings writeback happens at the end of EVERY tailoring run, for all users, without prompting — and because there is exactly one tailoring path, the writeback can never again be present in one entrypoint and missing in the other. Bonus: front-door tailoring now names each job folder from the shared canonicalizer (`norm_contracts --application-name`), the same source of truth vetting uses, instead of an ad-hoc "Company - Role" abbreviation. Comparison columns still fill only when the base artifact is readable enough to score (blank stays the honest "not measured", never an estimate); the base-name column always writes.
...
The generated `.docx` becomes `Cover-Letter-Draft - <Company - Role>.docx`, with **no** candidate-name prefix. It is a copy-paste source for the candidate's own template — what actually gets submitted is the exported PDF — and naming it after the candidate put it one character from the real deliverable sitting beside it.

**The structural fix matters more than the reshuffle.** The folder shape now lives in one module (`04-TAILOR/job_folder_layout.py`) with a CLI, and the agent prompts *call* it rather than describing a search order in prose. The 2026-08-04 rename left ~30 hardcoded path literals across JS and Markdown with no test coverage at all; going from two possible locations to four would have made that worse, because a code fallback chain is deterministic while an LLM working through a four-shape search order written in English is not. `cover-letter.js` alone drops from 16 hardcoded literals to 1. Two lookups stay deliberately distinct: `--find latest-letter` (highest `final-vN`, for revising) versus `--find coverletter-baseline` (pinned to `final.md`, for learning) — collapsing them would re-baseline the loop against a revision.

Back-compat is additive and permanent: readers resolve the current shape, the flat 2026-08-04 shape, legacy `_cl_work/`, cross-lane (a half-migrated folder is a real state), and the pre-lane `_extracted/`. The extraction cache check resolves by `MANIFEST.txt` rather than directory existence — without that, every already-extracted folder would re-extract into a second directory free to diverge from the first.

New coverage for what had none: `tests/test_layout_prompt_strings.py` statically checks that prompts resolve rather than assume, that no writer points at a legacy path, that lane spellings match the module, that work-dir paths are quoted (they contain spaces), and — the important one — pins the exact set of files naming `final.md`, so a new consumer of the learning baseline fails the build until a human looks at it.

`migrate_job_folder_layout.py` back-fills existing folders. Nothing ever moves outside its own job folder, so the worst outcome is files shuffled within one folder. It refuses to rename `final.md`, overwrite any destination, move a symlink, touch top-level deliverables, or start at all while a Dropbox "conflicted copy" is present; a folder is planned and validated entirely or skipped, never half-moved. Writing its version matching with globs was the bug the tests caught — `draft-v*.md` also matches `draft-v4-jessica-handwritten.md` and `final-v*.md` matches `final-v2-proposal.md`, both real files, neither an agent artifact. `_backups/` is excluded, since a folder named "pre-folder-rename" exists precisely to preserve the pre-rename shape.
...
Added a required rule to `00-job_application_agent.md`: **Selected Writing / Projects is always re-evaluated against the current JD**, even — especially — when the base is a finalized résumé for the same company or title. Reusing a base is a decision about the *chassis*, never about the links. "Use the base's section as-is" is a valid conclusion only after running the six-dimension evaluation for the current JD and independently landing on the same set, and the packet must say so explicitly. Silently listing the section among the untouched ones is now a defect, not a shortcut. The rule also requires reading the whole writing library rather than only the pieces the base already uses.

Second, generic lesson from the same investigation: a change **observed on a submitted artifact** is fact, not a hypothesis. This one had been correctly detected by reconcile but filed as "watch — single occurrence, needs confirmation," where it sat unused. Observed-on-submitted changes are exempt from the ≥2-occurrence promotion bar; only *inferences about what a change means* belong in the confirmation queue. Reinforces the existing §0 policy in `learning/reconcile-spec.md` — the failure was classification, not a missing rule.

## 2026-08-05 — New tracker status: "Down: Ruled Out" (human-only "I chose to pass")

Added a lifecycle status for the case where the candidate themselves decides not to apply to a role — for any reason (doesn't meet the bar well enough, comp not worth it, location, lost interest). It's human-only: the AI never assigns it. Added once to `norm_contracts.STATUS_VALUES` (the single source of truth), with the matching cell color in `make_rankings_xlsx.STATUS_COLORS` set to the same `A52A2A`/white red as the other "declined/closed" statuses, and a plain-English explanation on the workbook's Instructions tab. The status-color test (`STATUS_COLORS` keys must exactly equal `STATUS_VALUES`) keeps them in sync. No dropdown-limit issue: `inline_list_formula` already chunks the list into ≤240-char pieces concatenated with `&`, so the 13-value list renders fine (verified by regenerating a real workbook — dropdown + Instructions both carry the new value). Benefits every JAIL user.
...
## 2026-08-04 — `reconcile.js` brought up to the v2 §0 policy (code caught up to the spec)

The spec (`reconcile-spec.md §0`) and CLAUDE.md were upgraded to the fact-vs-inference policy earlier in the day, but the workflow that actually executes reconcile — `.claude/workflows/reconcile.js` — was still v1: it treated the candidate's own submitted bullets as "candidates for review" and kept the experience bank locked, so every reconcile still funneled the candidate's own words through the queue. That gap is exactly the "why do I have to do everything 2-5 times" failure mode — the policy was written down but the running code ignored it. Fixed by rewriting the Synthesize stage: (1) a §0 FACT-VS-INFERENCE rule now heads the inlined `RULES`; (2) Synthesize step 6 is unlocked to **append the candidate's verbatim shipped bullets/wording directly into the relevant `04-experience-bank.md` cluster** (source+date tagged, idempotent) and to **record every such application in `learning/decisions-log.md`**, alongside the existing summary/skills-library appends — while 01-profile, 05-summary-quick, 06-skills-quick and the voice-spec stay locked and only *inferences* go to the queue; (3) a new step 6b **processes and clears `learning/feedback-inbox.md`** (the hook's backstop) under the same §0 rule; (4) step 7 now requires the run to **surface every capture in chat** (applied-now vs surfaced-for-confirm, one line each) and emit a **loop-health line** (items applied · inferences awaiting confirm · prominently-featured experiences still missing a `03*-canonical.md` · last-feedback date). `meta.description` rewritten to state the v2 behavior. `node --check` passes.

---

## 2026-08-04 — Job folders decluttered: agent working artifacts move into `_JAIL Agent Work/`

A per-application folder (`<batch>/2 - Tailored Resumes/<Company - Role>/`) had accumulated enough agent scratch at its top level that the candidate had to hunt for the two files they actually review. Three renames, one consolidation: **`_cl_work/` → `_JAIL Agent Work/`** (leading underscore kept so it still sorts to the bottom; it is no longer cover-letter-only — it is now the single home for agent working artifacts), the cover-letter review packet **`application_coverletter_output - <Company> - <Role>.md` → `coverletter_agent_output - <Company> - <Role>.md`** *and* moved inside it, and the tailoring step's base-comparison sidecar **`comparison.json` → `resume_base_comparison.json`**, likewise moved inside it. Deliberately unchanged: `application_resume_output - <Company> - <Role>.md` keeps its name and stays at the top level (it is the candidate's primary review packet), as do the resume/cover-letter deliverables and the copied job capture `.txt`. The `- v2`/`- v3` version-suffix convention is untouched.

**Readers accept both shapes; writers emit only the new names.** This is the part that mattered: existing users have already-submitted archive folders sitting on disk in the old shape, and reconcile has to keep working on them unchanged. `extract_submission.py` gained a small resolver layer (`agent_work_dir` / `find_agent_artifact` / `coverletter_baseline` / `resume_base_comparison` / `coverletter_packet`) so the cover-letter diff baseline is looked up through a fallback chain — `_JAIL Agent Work/final.md`, then the legacy `_cl_work/final.md` — instead of a hard-coded path, and the diff header now names whichever it actually used. The comparison-sidecar resolver additionally falls back to `comparison.json` at the folder root, where the old shape put it. The cover-letter workflow's church-and-state versioning guard now checks **both** directories before concluding a job has no immutable baseline yet; missing that check would have silently clobbered the learning baseline of every pre-rename folder on its next run. New files always land in `_JAIL Agent Work/` even when the baseline they version past lives in a legacy `_cl_work/`, and no reader renames or migrates a legacy file as a side effect.

The comparison sidecar turned out to have no canonical definition anywhere in the tracked engine — it was being written ad hoc by the tailoring agent and only ever referenced in passing in this changelog. The job-folder layout is now stated once in `.claude/agents/job-applier.md` (Step 5a) and once in `00-job_application_agent.md` (Step 0), so "what belongs at the top level vs. in the work folder" is a rule rather than a convention that happened to hold.
