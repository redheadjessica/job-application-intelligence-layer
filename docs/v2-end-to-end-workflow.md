...
- **Done (2026-07-31 · ATS first-posted vs last-updated in the tracker):** the rankings contract is now **28 columns**. `Job Posted Date` was RENAMED to **`ATS First Posted Date`** (label only — existing values preserved, never recomputed), and a new **`ATS Last Updated Date`** sits immediately after it. Both are read back out of the capture's own ATS metadata (`Job Posted At:` / `Job Updated At:`) by the normalize pass — never JAIL's fetch date, a file date, a deadline, a syndicated-listing date, or anything inferred from posting language, and neither feeds scoring or ranking. **Deliberate asymmetry:** first-posted keeps the literal `Unknown` when unreadable (every posting has one), while last-updated stays **BLANK** when the ATS exposes none (many postings are never updated; Workable, Workday and LinkedIn never publish the field) — blank is a real answer, not a missing one. No mass back-fill: an existing row gains the value only from authoritative ATS metadata already stored in its capture. Sources that populate it: Ashby `updatedAt`, Greenhouse `updated_at`, Lever `updatedAt`, Rippling `updatedOn`, JSON-LD `dateModified`. Legacy CSVs migrate in ONE hop — both `Posted` and `Job Posted Date` land on the new label with values intact and gain the new column.
