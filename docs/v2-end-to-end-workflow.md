...
- **Done (post-V2 · Reconcile learning lanes):** `reconcile.js` + `reconcile-spec.md` gained the live-use July upgrades — **§12 cover-letter lane** (baseline `_cl_work/final.md` vs submitted PDF; content-heuristic letter detection incl. letter-as-page-3; plain-English candidates → the cover-letter feedback-queue, never the resume files), **§13 token-efficient extraction** (new `04-TAILOR/learning/extract_submission.py` caches PDF text + a sentence-level cover-letter diff into `_extracted/` so agents never re-read PDFs; candidate identity chrome stripped via the cover-letter `config.json`), **anecdote harvest** (candidate's own submitted stories → anecdote bank, direct entry) and an **answers lane** (submitted Q&A → new `04-TAILOR/learning/answer-bank.md`, template tracked), plus **§9b "submission = seal of approval"** (auto-register finalized bases into `02-resume-index.md`, auto-append verbatim summaries to `05a`; the human gate moves to the observed/inferred line). The tailoring engine + `job-applier` picked up the July output rules (no hard-wrapping, notes-first paste-ready bullet blocks, work-first section order, sibling-independence, summary angle-diversity / no-company-name / unique bullet openers, unusable-JD hard stop); `tailor-jobs.js` gained REBUILD-ON-STALE re-runs. `pypdf` joined `requirements.txt`.
