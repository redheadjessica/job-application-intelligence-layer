...
- **Done (V2A · Unit 1 — rename + template/instance split):** numbered folder architecture; `.template.md` / `.template.json` (tracked) vs generated instance files (gitignored); all path references updated; `__READY_TO_REVIEW__PRIVATE_GITIGNORED` date-shape batch guard; this doc.
...
- **Done (V2.2 · Tracker UX — the rankings output is now a job-search tracker, not just a score report):** the rankings CSV + XLSX use a 23-column **human-first** layout — editable columns (headers ending `? [You …]`: Applied Date, Status, Have Intro, Your Notes, Decline/Down Date) first, AI scoring/detail to the right. The `Lane`/`Lane Fit`/`Category` confusion is **fixed**: **`Lane`** is now the job's *actual* category in the job's own terms (e.g. "Product Strategy / AI Research"), and **`Lane Fit`** is the candidate-relative judgment ("Outside lanes (high)"). The buggy always-a-candidate-lane `Category` column and the dead blank `ClaudeStatus` column are **removed**. The XLSX adds: the header **frozen** (freeze `A2`); every cell **wrapped, centered, top-aligned**; an inline **Status dropdown** (the 12 lifecycle values; arrow shown) on the job rows; **auto-filter** so "Sort A→Z" is one click; lifecycle **Status colors** and **Lane stoplight** (p1 green / p2+ amber / Outside red); an **Instructions** tab; and — as a *separate block below the jobs* — a **section-color legend** (merged `A:J` bars) to copy from if you sort and want visual breaks. Crucially the **job rows have no merged cells, so they sort cleanly** (an earlier interleaved-divider design broke sorting; the legend is now a separate block, and the data ships ungrouped in score order so users can paste rows into their own tracker without duplicating dividers). Generated `Status` values now come from the dropdown vocabulary (`Skip` → `Apply Eventually: Or Skip It`; `run-batch`'s tailor-filter updated to match). Comp/Location **fit math** moved into `vet-jobs.js` and is written into the CSV; `make_rankings_xlsx.py` maps the labels → colors. **Superseded (2026-07-29 output-contract spec):** the single source is now `ENGINE__PUBLIC_GIT_TRACKED/03-VETTING/norm_contracts.py`, and `make_rankings_xlsx.py` re-normalizes and re-derives Working Location text, Lane, Comp Range, Comp Fit and the location colors **on read** (so old CSVs regenerate repaired) rather than only mapping labels. Also note the stoplight named here is **Lane Fit**, not `Lane`: `lane_color()` only distinguishes the Health bucket; it is `lane_fit_color()` that colors by matched-lane priority.
- **Done (post-V2 · Cover-letter module):** cover letters graduated from "later/maybe" to an optional module at `ENGINE__PUBLIC_GIT_TRACKED/04-TAILOR/cover-letter/` — template/instance split like everything else (tracked: `lint_cover_letter.py` [deterministic banned-phrase/punctuation/link-billboard/anti-smoothing gate, personal knobs via `config.json`], `make_cover_letter_docx.py` [config-styled .docx paste source], voice-spec / eval-rubric / formatting-spec / feedback-ledger / feedback-queue / writing-links / anecdote-bank templates, `exemplars/README.md`; gitignored: the bare-name instances). New `/cover-letter-intake` skill (studies the person's past letters + writing, stages voice spec + exemplars + anecdotes + config for approval, mirrors `/intake`'s staged-review discipline); new `cover-letter` workflow (draft → lint → dual adversarial eval [Fit + Voice vs the person's GOLD exemplar, mandatory self-pushback] → surgical revise under a preservation lint → .docx + link QA + compact packet); `cover-letter-writer` / `cover-letter-evaluator` agents. Reconcile's letter learning loop is wired in (see the next bullet).
- **Done (post-V2 · Reconcile learning lanes):** `reconcile.js` + `reconcile-spec.md` gained the live-use July upgrades — **§12 cover-letter lane** (baseline `_cl_work/final.md` vs submitted PDF; content-heuristic letter detection incl. letter-as-page-3; plain-English candidates → the cover-letter feedback-queue, never the resume files), **§13 token-efficient extraction** (new `ENGINE__PUBLIC_GIT_TRACKED/04-TAILOR/learning/extract_submission.py` caches PDF text + a sentence-level cover-letter diff into `_extracted/` so agents never re-read PDFs; candidate identity chrome stripped via the cover-letter `config.json`), **anecdote harvest** (candidate's own submitted stories → anecdote bank, direct entry) and an **answers lane** (submitted Q&A → new `PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/learning/answer-bank.md`, template tracked), plus **§9b "submission = seal of approval"** (auto-register finalized bases into `02-resume-index.md`, auto-append verbatim summaries to `05a`; the human gate moves to the observed/inferred line). The tailoring engine + `job-applier` picked up the July output rules (no hard-wrapping, notes-first paste-ready bullet blocks, work-first section order, sibling-independence, summary angle-diversity / no-company-name / unique bullet openers, unusable-JD hard stop); `tailor-jobs.js` gained REBUILD-ON-STALE re-runs. `pypdf` joined `requirements.txt`.
- **Done (2026-07-30 · The JOB SNAPSHOT capture format is now the standard):** the saved job-post `.txt` preamble is a HUMAN-READABLE contract, not implementation language. Sections in fixed order — `JOB SNAPSHOT` (Company / Role / Job Posting URL / `Job Posted At:` / `Job Updated At:`, the dates always present and `Unknown` when the source exposed none) · `WORK DETAILS` (Employment / Work Arrangement / Working Location(s) / optional `Location Preference:` when the employer STATES one / Office Expectation) · `COMPENSATION` (Base Salary inline for a single range or bulleted per geo/level band, abbreviated thousands; Additional Compensation; Benefits — blank-line separated) · `APPLICATION QUESTIONS WORTH PREPARING` (`1. <question> [Required|Optional]` + indented bracketed context lines, or `None Found.`) · the job text byte-for-byte between the unchanged `--- JOB TEXT START/END ---` markers · then `ORIGINAL CAPTURE DETAILS` and, on a re-fetch, `LATEST CAPTURE DETAILS` BELOW the end marker (both labeled `Captured At:`, rendered in Eastern Time). Per-field states use honest sentence-case distinctions (`Employer did not mention <X>.` / `Could not verify.` / `Conflicting employer information: A vs B.` / `<X> mentioned, but details not provided.` / `Not Specified`), and a `Verification:` line summarizes Job Description / Compensation / Working Location with ✓ ✗ — ⚠. Employer list structure survives into the body (`<ul>/<ol>/<li>` → `- item` / `1. item`, nested lists indented; `<br>` is a line break, `<br><br>` a paragraph break); nothing is ever invented from prose. Machine consumers read the manifest's `field_status`, not the txt; the only machine-anchored lines in the file are the two snapshot dates, and `norm_contracts.posted_date_from_capture()` parses BOTH the new `Job Posted At:` and the legacy `Posted:` form, bounded to the pre-body region.
- **Done (2026-07-30 · ORIGINAL vs LATEST capture, and the durable capture-history registry):** batch manifests are per-batch MIRRORS; the source of truth for a posting's capture history is a gitignored registry at `PRIVATE__YOUR_FILES_GITIGNORED/02-PREP__YOUR_PRIVATE_INFO/capture-history-registry.json`, keyed by canonical posting identity (`<ats>:<board>:<posting id>` when recognizable — employer deep links, `?gh_jid=` params and board URLs all alias to one key — else the normalized URL). Per posting: an **immutable** `original_capture` (the earliest successful fetch), a `latest_capture` (the most recent successful fetch — a failed request never replaces it), and append-only history. Local re-renders/previews and skipped URLs are NOT captures. "ORIGINAL" in a capture file means *the earliest capture JAIL can establish from its durable records*: `backfill_capture_registry.py` seeds the registry read-only from existing manifests (including archives), marking such originals `backfill-earliest-known`. Writes are atomic (tmp + `os.replace`) under an advisory sidecar lock, and a staging/canary worker can be isolated onto its own shard (`registry_path=` / `JAIL_CAPTURE_REGISTRY`) then merged post-validation with `merge_registry_shards(global_path, shard_paths, accepted_keys)` — only accepted identities merge, the earliest original wins, and repeated merges are idempotent.
- **Done (2026-07-30 · The 27-column rankings contract; extended to 28 on 2026-07-31 — see the next bullet):** the rankings CSV and XLSX share ONE authoritative 27-column order, defined once in `norm_contracts.CONTRACT_TEMPLATE` / `resolve_contract_headers()`: Applied Date? [You Fill In] · Status? [You Change] · Lane · Company · Job Post Title + Link · Working Location · Comp Range · Have Intro? [You Add] · Your Notes? [You Add] · Decline/Down Date? [You Add] · the five score columns · Job Posted Date · Top Reasons Notes · Top Concerns Notes · Profile Score Notes · Your Desire Score Notes · Comp + Lifestyle Fit Notes · Lane Fit · Comp Fit · Data Completeness · Job File · Tailored? (Base Resume) · Cover Letter Drafted?. **`Location Fit` was REMOVED** as redundant with `Working Location` — the canonical grammar already encodes remote/metro/cadence, both cells shared the same four-hex fill, and a second derived label was one more thing to keep in sync; `Working Location` keeps those four hexes unchanged. Migration happens at the WRITER: `normalize_rankings_csv` renames legacy headers, drops the removed column, inserts missing ones (back-filling `Data Completeness` with the shared `fallback_completeness`), and reorders — joined by header NAME so no column's data shifts, and never dropping a column someone added themselves. `Job Posted Date` is never blank (the literal `Unknown` when unverified; never the capture date, never inferred). Only columns marked `[You Fill In]` / `[You Change]` / `[You Add]` are human-managed — a bare `?` does not imply it, and `Tailored? (Base Resume)` / `Cover Letter Drafted?` are filled by the pipeline via `update_rankings_row.py`.
- **Done (2026-07-31 · ATS first-posted vs last-updated in the tracker):** the rankings contract is now **28 columns**. `Job Posted Date` was RENAMED to **`ATS First Posted Date`** (label only — existing values preserved, never recomputed), and a new **`ATS Last Updated Date`** sits immediately after it. Both are read back out of the capture's own ATS metadata (`Job Posted At:` / `Job Updated At:`) by the normalize pass — never JAIL's fetch date, a file date, a deadline, a syndicated-listing date, or anything inferred from posting language, and neither feeds scoring or ranking. **One shared convention** (revised 2026-07-31, superseding an earlier blank-vs-`Unknown` split): both columns read the literal `Unknown` when the ATS provides no date, and **neither is ever blank** — two adjacent date columns with two different empty conventions is a trap, and a blank tracker cell reads as unfinished work. So Workable, Workday and LinkedIn postings (whose boards never publish an update field) show `Unknown` there. No mass back-fill: an existing row gains the value only from authoritative ATS metadata already stored in its capture. Sources that populate it: Ashby `updatedAt`, Greenhouse `updated_at`, Lever `updatedAt`, Rippling `updatedOn`, JSON-LD `dateModified`. Legacy CSVs migrate in ONE hop — both `Posted` and `Job Posted Date` land on the new label with values intact and gain the new column.
- **Carried live caveats (need the Python venv):** Unit 4 live network fetch still needs a real-dependency sanity test — tracked in `testing-and-caveats.md`. The cover-letter .docx generator needs `python-docx` (in `requirements.txt`).
...
**Future roadmap:** V3 resume creation/rendering · V4 optional API-backed sourcing · later/maybe formatted resume export, Google Sheets export, hosted version, stronger application-answer generation. *(Cover letters have since shipped as the optional post-V2 module — see the status list above.)*
...
6. **Staged intake review** — Claude writes proposals into `__READY_TO_REVIEW__PRIVATE_GITIGNORED/MM-DD-YY - Intake Review/` (with `START HERE.md` and numbered review files), not directly into canonical files, and summarizes in chat.
...
11. **Rank jobs** — into `__READY_TO_REVIEW__PRIVATE_GITIGNORED/MM-DD-YY/` with `START HERE.md`, `0 - Prep Report/`, `1 - Rankings/`, `3 - Source Material/`. The rankings are a **tracker**: editable columns first (Applied Date, Status, Have Intro, Notes, Decline/Down Date), then the job's **Lane** (what the role is), company, title/link, working location, comp, final score, the four sub-scores, the **Comp + Lifestyle Fit Notes** rationale (the prose companion sitting immediately after that dimension's score), the desire/profile score notes, top reasons, top concerns, and the candidate-relative **Lane Fit / Comp Fit** (plus `Data Completeness`, `Job File`, `Tailored? (Base Resume)`, `Cover Letter Drafted?`). **`Location Fit` was removed on 2026-07-30** as redundant with `Working Location` — see the 27-column contract bullet above. Rows ship **grouped into Status sections**. Coloring: **Comp Fit / Lane Fit / scores** are candidate-relative (comp vs floor/target via the midpoint rule, Lane **Fit** vs matched-lane priority, scores vs rubric). **Working Location is deliberately NOT config-driven**: since the 2026-07-29 output-contract spec they use four fixed hexes derived deterministically from the normalized location text — green for genuinely-available remote, yellow for an acceptable home-metro office at exactly 1–3 days, orange for unknown / unknown cadence / >3 days / open-ended minimums, red for required in-person outside the acceptable geography. No grey. See `ENGINE__PUBLIC_GIT_TRACKED/03-VETTING/norm_contracts.py`.
12. **User chooses jobs to tailor** — selected, top N, one at a time, or a manual set.
13. **Tailor selected jobs** — into `__READY_TO_REVIEW__PRIVATE_GITIGNORED/MM-DD-YY/2 - Tailored Resumes/Company - Role/`, each with the original job post, the application-resume-output Markdown, recommended base, questions for the candidate, risks/truth checks, tailoring notes.
14. **Final review outside JAIL** — the user's judgment, plus optionally another AI assistant or a trusted human. Optional, **not productized** in V2 (no special packets, no ChatGPT-specific flow). **Cover letters** are the post-V2 exception: once `/cover-letter-intake` has been run, `cover-letter {job: "…"}` drafts + adversarially evaluates a letter into the job folder (see `ENGINE__PUBLIC_GIT_TRACKED/04-TAILOR/cover-letter/README.md`).
...
16. **Move completed folder to archive** — after submission, **move** (not copy) the job folder into the archive (default `PRIVATE__YOUR_FILES_GITIGNORED/05-SUBMITTED-APPLICATIONS__YOUR_PRIVATE_INFO/<year>/`, configurable). `__READY_TO_REVIEW__PRIVATE_GITIGNORED` is a workspace, not the long-term record; don't delete a batch before archiving its completed applications.
...
flowchart TD
  ...
  M --> IN["Run /intake"]
  IN --> SR["Staged review: __READY_TO_REVIEW__PRIVATE_GITIGNORED/MM-DD-YY - Intake Review/"]
  ...
  SUB --> AR["/archive -> PRIVATE__YOUR_FILES_GITIGNORED/05-SUBMITTED-APPLICATIONS__YOUR_PRIVATE_INFO/"]
...
ENGINE__PUBLIC_GIT_TRACKED/04-TAILOR/  (public: engine + *.template.md)   ·   PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/  (your instances)
  ...
  learning/
    ...
    extract_submission.py           (engine — cached PDF extraction for reconcile)
    learning-ledger.template.md     →  learning-ledger.md       (instance)
    source-update-queue.template.md →  source-update-queue.md   (instance)
    answer-bank.template.md         →  answer-bank.md           (instance, written by /reconcile)
  cover-letter/                     (optional module — see its README)
    README.md · lint_cover_letter.py · make_cover_letter_docx.py   (engine)
    config.template.json            →  config.json              (instance)
    voice-spec / eval-rubric / formatting-spec / feedback-ledger /
      feedback-queue / writing-links / anecdote-bank .template.md → instances
    exemplars/README.md             →  exemplars/*.md           (instances)

PRIVATE__YOUR_FILES_GITIGNORED/05-SUBMITTED-APPLICATIONS__YOUR_PRIVATE_INFO/   (gitignored archive; default; year subfolders created lazily)

__READY_TO_REVIEW__PRIVATE_GITIGNORED/   (review hub: MM-DD-YY job batches + "MM-DD-YY - Intake Review", etc.)
...
## 9. Approved truths & boundary rules (`PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/03-approved-truths-and-boundary-rules.md`)
...
- **Default archive:** in-repo, gitignored `PRIVATE__YOUR_FILES_GITIGNORED/05-SUBMITTED-APPLICATIONS__YOUR_PRIVATE_INFO/<year>/`, configurable via `jail.config.json` (point it at a cloud-synced folder for durability if you prefer).
...
5. **Candidate-relative ranking** (done) — `vet-jobs` inlines `jail.config.json` (sharper `practicality_score`) + `lane_fit`; colorizer reads config → candidate-relative Comp Fit + Lane Fit columns (Location Fit color became spec-fixed rather than config-driven on 2026-07-29, and the column itself was removed on 2026-07-30 as redundant with Working Location), status-aligned `final_score`, neutral fallback + quarantine note; intake captures arrangement ratings. **V2.2** reworked the output into a human-first **tracker**: the Lane vs Lane Fit fix, an inline Status dropdown + lifecycle colors, freeze + wrap/center/top, auto-filter, a separate section-color legend block (the job rows stay sortable), an Instructions tab, and removal of the `Category` / `ClaudeStatus` columns.
