...
- **Ranking spreadsheet render (Unit 5)** ✅ — the real `vet-jobs` workflow scored 4 synthetic posts and `ENGINE__PUBLIC_GIT_TRACKED/03-VETTING/make_rankings_xlsx.py` rendered the `.xlsx` via `openpyxl`. Confirmed: the candidate-relative fit columns + colors (comp vs target/floor, location vs the candidate's per-arrangement ratings), the status-aligned final score, and the quarantine note all render correctly. *(Superseded 2026-07-29: Comp Fit uses the midpoint rule and Working Location / Location Fit use four fixed spec hexes from the normalized location text — the per-arrangement ratings no longer drive any color. Covered by real pytest regression tests in `ENGINE__PUBLIC_GIT_TRACKED/03-VETTING/tests/`, which assert the written XLSX cell fills.)* Committed artifact: `docs/examples/jordan-lee-demo/expected-outputs/rankings.xlsx`. *(The column layout was reworked in **V2.2** — see the V2.2 note below.)*
...
- **Fit-math single source** ✅ — the comp/location label functions ported to `vet-jobs.js` produce labels **identical** to the former Python implementation across all four synthetic rows (checked directly); `make_rankings_xlsx.py` now only maps labels → colors. *(Superseded 2026-07-29: the single source moved to `03-VETTING/norm_contracts.py`, which the JS workflow shells out to and `make_rankings_xlsx.py` imports — the spreadsheet build now re-normalizes text and re-derives Comp Fit + location colors on read, so a stale CSV regenerates repaired.)*
