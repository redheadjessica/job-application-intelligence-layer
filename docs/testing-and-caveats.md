...
- **Ranking spreadsheet render (Unit 5)** ✅ — the real `vet-jobs` workflow scored 4 synthetic posts and `03-VETTING/make_rankings_xlsx.py` rendered the `.xlsx` via `openpyxl`. Confirmed: the Comp Fit / Location Fit / Lane Fit columns, candidate-relative colors (comp vs target/floor, location vs the candidate's per-arrangement ratings), the status-aligned `final_score`, and the A1 quarantine note all render correctly. Committed artifact: `docs/examples/jordan-lee-demo/expected-outputs/rankings.xlsx`.
