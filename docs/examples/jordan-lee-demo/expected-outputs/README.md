...
| `rankings.xlsx` | the colored ranking spreadsheet (**hero artifact**) | **Real `03-VETTING/make_rankings_xlsx.py`** rendered it from `rankings.csv` + `jail.config.json` (candidate-relative comp/location colors) |
...
Thornbury **93** (Apply ASAP, 🟢comp/🟢loc) · Flintlock **64** (Backup, 🟢comp/🔴loc) · Lanternleaf **62** (Backup, 🔴comp/🟢loc) · Lyceum AI **33** (Skip — green comp+loc but weak fit) · Hatchwing **quarantined**.
