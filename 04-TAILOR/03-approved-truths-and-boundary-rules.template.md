...
> resume to describe with consistent, canonical wording. Otherwise leave it as-is or delete it.**
>
> This file exists for one situation: you have a *current* venture / role / company whose framing
> matters enough that you do not want the resume to drift or quietly overclaim it from one
> application to the next. Examples: a startup you founded that is pre-scale, a new role where the
> title is doing a lot of work, an early-stage product with no revenue or users yet. If that is you,
> fill in the placeholders below so every tailored resume describes it the same approved way. If you
> have no such "current work" worth pinning down, this file does nothing — skip it.

<!-- intake: If you keep this file, the rest of the system can read it on every generation run as the
...
## Why This Work Matters (positioning)

<!-- intake: One short list of why this current work is worth featuring. This sets how aggressively it
should appear. Delete the file if the honest answer is "it isn't worth featuring." -->

1. {{REASON 1 — e.g. "It is current work."}}
2. {{REASON 2 — e.g. "It demonstrates a hands-on skill the target roles value."}}
3. {{REASON 3 — e.g. "It strengthens positioning for a specific kind of role."}}
...
`{{HEADER — Title @ Company (status label) • Year - Present}}`

<!-- intake: e.g. "Founder & Product Lead @ Acme (Early Access) • 2026 - Present". The status label
(if any) is doing important honesty work — see Claim Boundaries below. -->

**Status label and what it means (the claim boundary):**

<!-- intake: If your header carries a stage/status label (e.g. "Early Access", "Beta", "Pilot",
"Stealth"), define exactly what it does and does NOT imply, so the resume can lean on it without
overclaiming. If there is no label, delete this block. -->
...
<!-- intake: Write 2-4 pre-approved bullets here, each one interview-defensible and true today. The
resume should pull from this set rather than writing fresh claims. Tag each with what angle it leads
with so the agent can pick by role fit. Keep them in your own tense rule (see below). -->

1. {{BULLET 1 — what the work IS + your core current activity}}
2. {{BULLET 2 — the strategy / depth angle}}
3. {{BULLET 3 — the systems / end-to-end / outcome angle, strongest for closely-aligned roles}}
...
<!-- intake: List the handful of phrases/concepts that should survive even when the section is
compressed — the things that make this work distinctive and that you want stated consistently. -->
...
<!-- intake: If there is a way people commonly mis-categorize this work (too narrow, too broad, wrong
domain), say so here so the resume frames it correctly. Delete if not applicable. -->

{{FRAMING — e.g. "Do not characterize this as exclusively an X product; it is broader than that."}}
...
- {{TRANSFERABLE ANGLE 2}}
- {{TRANSFERABLE ANGLE 3}}
...
- Bias toward clear, modern framing relevant to the role.
- Use role-appropriate emphasis.
- Preserve your tone and writing rules.
...
When presenting a tailored resume draft, briefly note:
- how this work was positioned for the role (which mode),
- what was compressed or cut to make room,
- any suggested future updates to this section to review later.
