export const meta = {
  name: 'reconcile',
  description: 'Post-application learning (v2, §0 fact-vs-inference): reconcile completed submitted applications (compare the agent\'s recommendation vs the finalized submitted resume) AND process the feedback inbox. The candidate\'s OBSERVED content — bullets/wording they actually shipped and feedback they state in chat — is FACT, applied to the Evidence layer immediately (experience bank / summary + skills libraries) and recorded in learning/decisions-log.md, no review gate. ONLY the system\'s INFERENCES about what a change means are queued for confirmation (learning/source-update-queue.md). Writes a per-folder reconcile report and surfaces every capture in chat. Never edits the locked generation files (01-profile, 05-summary-quick, 06-skills-quick, voice-spec). Implements ENGINE__PUBLIC_GIT_TRACKED/04-TAILOR/learning/reconcile-spec.md §0.',
...
- ⭐ §0 FACT-VS-INFERENCE (governs everything below). The candidate's OBSERVED CONTENT is FACT, applied to canon IMMEDIATELY with no review gate — both (a) feedback they state in chat (the invoking-message text AND the feedback inbox at PRIVATE__…/04-TAILOR__YOUR_PRIVATE_INFO/learning/feedback-inbox.md) and (b) the actual bullets, wording, base choice, and section structure on the submitted résumé. A bullet they shipped is approved and final — NEVER ask them to confirm their own résumé back to them. The SYNTHESIZE stage applies this observed content directly to the Evidence layer it governs (experience bank / summaries / skills) or Behavior Rules, records it in decisions-log.md, and echoes each capture in chat. The ONLY thing that stays gated (the queue) is YOUR INFERENCE about what a change MEANS — surfaced as a plain question ("you dropped X on the last few — retire it, or keep for some cases?"). Nothing below overrides this.
...
- COVER-LETTER LANE (spec §12; skip if the cover-letter module isn't set up — no PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/cover-letter/feedback-queue.md instance): the agent-side baseline is the ORIGINAL un-versioned "_JAIL Agent Work/final.md" (legacy folders: fall back to "_cl_work/final.md"). ⭐ BASELINE RULE (candidate, 8/5/26 — do NOT second-guess): the baseline is ALWAYS the FIRST finalized letter — the un-versioned final.md, the first one created, BEFORE any feedback — NEVER a "final-v2/v3/…" revision. This is deliberate and mirrors the résumé exactly (original application_resume_output vs submitted PDF). The intended signal is: where the agent STARTED (v1, before the candidate gave any feedback) → what the candidate ACTUALLY SUBMITTED (the PDF). Intermediate versions are IGNORED. Do NOT "reduce noise" by switching the baseline to the newest final-vN — the v1→submitted delta IS the point, even when part of it came from revisions the candidate requested along the way. The extract_submission.py resolver already picks the un-versioned original; never override it. Same observed/inferred firewall. Frame every candidate lesson in plain English: "You did Y on the [Company] letter — on purpose? Make it the default?" with before/after text. Cover-letter lessons are returned separately (cover_letter_findings) — they route to PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/cover-letter/feedback-queue.md, NEVER to the resume learning files. If the letter predates the cover-letter workflow (no baseline), record observed style patterns only, marked "no-baseline".
- EXTRACTION-FIRST (token discipline, spec §13): ALWAYS run .venv/bin/python3 ENGINE__PUBLIC_GIT_TRACKED/04-TAILOR/learning/extract_submission.py "<folder>" first (cached after the first run). It prints "EXTRACTION_DIR: <path>" as its first line — that directory is where the results are for THIS folder (older folders keep theirs in a top-level "_extracted/"; never assume a location, always use the printed one; below, $EXTRACTION_DIR means that path). Then read $EXTRACTION_DIR/*.txt and $EXTRACTION_DIR/coverletter-diff.txt INSTEAD of the PDFs — the diff IS the cover-letter signal (interpret it; don't re-diff). Open a PDF directly only if the manifest shows extraction failed or classified nothing. Read screenshot images ONLY if the manifest lists them AND submitted-answers.txt lacks their content; if you do, append the transcription to $EXTRACTION_DIR/submitted-answers.txt marked "[transcribed from <filename>]" so no future run ever reads the image again.
- ANECDOTE HARVEST (spec §13; skip if no PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/cover-letter/anecdote-bank.md instance): from the submitted cover letter and answers, extract every personal story / lived detail (family, history, hobbies, personal product use, origin stories) — ESPECIALLY ones the candidate added by hand (in the diff) or that appear nowhere in the anecdote bank. These are the candidate's own words = observed facts; they go DIRECTLY into the bank (tagged with source), not through the queue. Also report bank stories that were REUSED (for Used-in tracking). Never harvest professional claims (those live in the experience bank) — only the personal/lived material.
- ANSWERS LANE (spec §13): from $EXTRACTION_DIR/submitted-answers.txt, extract each question + the candidate's answer as an archetype pair (why-this-company, how-you-use-AI, etc.) for PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/learning/answer-bank.md. Condense answers >150 words to argument + anecdote slugs. Answers are harvest-only (no baseline exists).`
...
- The manifest + extracted texts: "$EXTRACTION_DIR/MANIFEST.txt", "$EXTRACTION_DIR/submitted-resume.txt", "$EXTRACTION_DIR/submitted-coverletter.txt", "$EXTRACTION_DIR/submitted-answers.txt", and "$EXTRACTION_DIR/coverletter-diff.txt" (if present)
- Scraped JD: "${f.jd_file}"
- Agent recommendation: "${f.output_md}"
- PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/cover-letter/anecdote-bank.md (slugs only — to tell new stories from reuses; skip if it doesn't exist)
...
- ${PRIV_APP}/learning/decisions-log.md        (the candidate's OWN applied rulings — read FIRST; never re-litigate anything here)
- ${PRIV_APP}/learning/feedback-inbox.md       (raw chat feedback captured by the hook since the last run — process it under §0, then clear the processed entries)
...
4b. COVER-LETTER CANDIDATES (spec §12; skip if the cover-letter module isn't set up): for every result whose cover_letter_findings.candidates is non-empty, APPEND them to PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/cover-letter/feedback-queue.md (folder-keyed, status: pending, idempotent — skip folders already present). Keep each in its plain-English "You did Y on the [Company] letter — on purpose? Make it the default?" form with the before/after text. NEVER write to PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/cover-letter/feedback-ledger.md (promotion is the candidate's manual confirmation) and NEVER route cover-letter lessons into the resume learning files.
4c. ANECDOTE BANK (spec §13; skip if no anecdote-bank instance): merge all anecdotes into PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/cover-letter/anecdote-bank.md following its entry format. new=true -> add a full entry (Status: confirmed, submitted — <Company> <date>) unless a same-story entry already exists (then just update Used in). new=false -> append "<Company> — <Role> (<date>)" to that slug's Used in line if not already listed. These are the candidate's own submitted words — direct entry, no queue.
...
6. §0 DIRECT APPLICATION of the candidate's OBSERVED VERBATIM CONTENT (their submitted artifacts + their chat feedback are FACT — apply now, no queue): you MAY and SHOULD (a) add base registrations to 02-resume-index.md per step 5; (b) APPEND each folder's verbatim finalized summary (role-archetype labeled, folder-keyed) to 05a-summary-library.md and any clear reusable skills observation to 06a-skills-library.md; and (c) ⭐ APPEND the candidate's OWN submitted résumé bullets/wording — a new variant they shipped, a reworded bullet, a detail they added — DIRECTLY into the relevant 04-experience-bank.md cluster, tagged with source + date and idempotent (skip if that exact wording is already present). These are the candidate's verbatim words, not your inference, so they are not gated. Record every such application in ${PRIV_APP}/learning/decisions-log.md as a dated entry (Type: Evidence; Applied to: <file>; Status: applied). Create any instance from its template if missing. STILL LOCKED (never edit without the queue): 01-profile.md, 05-summary-quick.md, 06-skills-quick.md, the cover-letter voice-spec, the cover-letter feedback-ledger; and INFERRED generalizations (a RULE you're proposing, not their words) always go through the queue — they approve rules, not their own content. 03-approved-truths-and-boundary-rules.md: a verbatim do-not-claim boundary the candidate stated in chat may be recorded (log it), but never infer a new boundary from a résumé change.
6b. FEEDBACK INBOX (§0): read ${PRIV_APP}/learning/feedback-inbox.md. For each entry that is real tailoring/résumé/skills/summary feedback (ignore data dumps, job URLs, routine chatter), apply the EXPLICIT parts directly to the layer they govern (per step 6) + log in decisions-log.md, and route only genuine INFERENCES to the queue as questions. After processing, mark the processed entries done (or clear them) so they are not re-processed. If the inbox is empty or absent, skip.
7. Keep the ledger and queue tight and honest; flag confounds explicitly. SURFACE every capture: your returned notes must list, one line each, what was applied-now (candidate content/decision → which file) vs surfaced-for-confirm (your inference → queue), so the workflow can echo them to the candidate. End with a LOOP-HEALTH line: "<N> candidate items applied · <M> inferences awaiting confirm · <K> prominently-featured experiences still lacking a 03*-canonical.md · last feedback captured <date>".

Write the updated files, then return the structured summary.`,
