...
- COVER-LETTER LANE (spec §12; skip if the cover-letter module isn't set up — no PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/cover-letter/feedback-queue.md instance): the agent-side baseline is "_cl_work/final.md" — the candidate NEVER edits the generated .docx, so baseline vs submitted delta IS their feedback. Same observed/inferred firewall. Frame every candidate lesson in plain English: "You did Y on the [Company] letter — on purpose? Make it the default?" with before/after text. Cover-letter lessons are returned separately (cover_letter_findings) — they route to PRIVATE__YOUR_FILES_GITIGNORED/04-TAILOR__YOUR_PRIVATE_INFO/cover-letter/feedback-queue.md, NEVER to the resume learning files. If the letter predates the cover-letter workflow (no baseline), record observed style patterns only, marked "no-baseline".
