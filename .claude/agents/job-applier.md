...
5a. **What goes at the job folder's top level, and what goes in `_JAIL Agent Work/`.** The top level is the candidate's review surface and stays uncluttered: the resume/cover-letter deliverables, the copied job capture `.txt`, and `application_resume_output - <Company> - <Role>.md` (their primary review packet). **Everything else you produce is an agent working artifact and goes in a `_JAIL Agent Work/` subfolder** — the leading underscore sorts it to the bottom. That includes the resume base-comparison sidecar, which is named `_JAIL Agent Work/resume_base_comparison.json`, plus every cover-letter intermediate the cover-letter agents write there (`draft-v1.md`, `eval-1.md`, `final.md`, `coverletter_agent_output - <Company> - <Role>.md`). Use `mkdir -p` and quote the path — it contains spaces.

   **Reading an existing folder (back-compat).** Folders created before 2026-08-04 use the old shape: a `_cl_work/` directory, `comparison.json` at the folder root, and `application_coverletter_output - … .md` at the folder root. When you read, accept either shape and prefer the new one. When you write, emit ONLY the new names — never re-create `_cl_work/` or `comparison.json`, and never rename or move a legacy file as a side effect of a tailoring run.
