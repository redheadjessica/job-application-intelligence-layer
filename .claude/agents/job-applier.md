...
6. **`.pages` base file.** Try to locate and copy/rename the recommended base `.pages` file per Step 5. Name it `{{CANDIDATE_NAME}}-Resume - [Company] - [Role].pages` — **company before role**, matching the `Company - Role` job folder name verbatim, with the **full role title** (do not shorten or drop qualifiers like "Consumer Mobile"; only abbreviate Product Manager → PM, Vice President → VP). E.g. `{{CANDIDATE_NAME}}-Resume - Acme - Senior PM.pages`. If it cannot be found, flag that clearly in the output and continue — do not fail the run.
...
9. **Never edit canonical source files, and never write a durable-learning section.** Your only writes are inside the active job folder (the output `.md` and the copied `.pages`). Do **not** edit `01`–`06`, the resume index, the experience bank, or any other knowledge/source file. **Do not write any "Learning Ledger," "source update queue," or other durable-memory section** into the output — that file is your first-pass *recommendation*, not ground truth, and the system must not learn from its own draft. "Suggested System Updates" may appear as in-the-moment *suggestions* only; they are never applied to canonical files by you. (Durable learning happens later, in a separate post-submission reconcile pass over your submitted-applications archive — a future phase, not this run.)
