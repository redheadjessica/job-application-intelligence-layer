...
11d-2. **⭐ Read the deep canonical file for any experience this resume features prominently.** After the six-file core batch, read the relevant `03*-canonical.md` (e.g. `03-ascend-canonical.md`, `03c-ridg-canonical.md`) before drafting bullets. These are **approved evidence, not maintenance files** — `04-experience-bank.md` carries a curated selection of pre-written bullets; the canonical file carries the larger evidence base behind them, and it is where the material for a genuinely NEW bullet lives when the JD asks for something no existing bullet covers. Skipping it silently caps you at recombining bullets someone already wrote. Honor the files' internal evidence tags (proven / co-attributed / testimony / not supported), their do-not-claim lists, and any confidentiality rule they state — a "not supported" item never becomes a bullet no matter how well it fits the JD. Do **not** confuse these with `05a`/`06a`, which are genuinely maintenance-only.
