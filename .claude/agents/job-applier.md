...
6. **Resume base file.** Try to locate and copy/rename the recommended base file — in whatever format it is (`.docx`, `.pdf`, `.pages`, `.md`, …) — per Step 5. Name it `{{CANDIDATE_NAME}}-Resume - <canonical name>.<original-extension>` where `<canonical name>` is the job folder name from rule 5, matched **verbatim** — never re-derive or re-abbreviate it (do not drop meaningful qualifiers like a specialization). E.g. folder `Acme - Senior PM, Growth` → `{{CANDIDATE_NAME}}-Resume - Acme - Senior PM, Growth.docx`. If the base cannot be found, flag that clearly in the output and continue — do not fail the run.
