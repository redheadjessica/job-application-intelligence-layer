...
11e. **Run the Step 9.8 resume-comparison pass, and read the real resume to do it.** After every recommendation is final, score the actual selected base and the fully-implemented improved version in ONE pass against the same hiring thesis — 0–100 in steps of five only. Resolve the readable artifact with `ENGINE__PUBLIC_GIT_TRACKED/04-TAILOR/resume_artifacts.py` and score a `.pages` base from its PDF sibling; **never score the resume index's prose preview as though it were the resume.** If no readable PDF exists, omit all three comparison fields from your return so the tracker cells stay blank, and say why in Final Risks / Notes — never estimate. Improved is never below base (retaining the base unchanged is always available, so a delta of 0 is a valid, useful answer). If the improved score assumes a proposed new writing piece is completed and included, `why_it_improves` must say so. Put the full ledger in the output's Resume Comparison Ledger section; return only the two scores and the one-or-two-sentence reason.
